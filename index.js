...
async function getCompanions(contributions, bannedKeywords) {
  ...
  const now = new Date()
  const companionsResponse = await collectPaginatedAPI(notion.databases.query, { database_id: '40cc4fea-6f70-4699-b463-aa36083a6f99' });
  const companions = companionsResponse.filter(x => contributions.includes(x.id) && x.properties.Picture.files.length > 0 && (x.properties.Retirement.date == null || !(new Date(x.properties.Retirement.date.start) < now && (x.properties.Retirement.date.end == null || new Date(x.properties.Retirement.date.end) > now )))).map(x => ({id: x.id, name: x.properties.Name.title[0].plain_text, url: x.properties.Website.url, services: x.properties.Services.multi_select.map(y => y.name), race: x.properties.Race.multi_select.map(y => y.name), gender: x.properties.Gender.multi_select.map(y => y.name), catersto: x.properties['Caters to'].multi_select.map(y => y.name), age: x.properties.Age.multi_select.map(y => y.name), body_type: x.properties["Body type"].multi_select.map(y => y.name), height: x.properties.Height.multi_select.map(y => y.name), tattoos: x.properties['Tattoos & mods'].multi_select.map(y => y.name), body_hair: x.properties['Body hair'].multi_select.map(y => y.name), tagline: x.properties['Tagline (EN)'].rich_text.length > 0 ? x.properties['Tagline (EN)'].rich_text[0].plain_text : null, keywords: x.properties.Keywords.rich_text.length > 0 ? x.properties.Keywords.rich_text[0].plain_text.toLowerCase() : "", location: x.properties.Location.multi_select.map(y => y.name)}))

  // Test: companionsResponse.find(x => x.properties.Name.title[0].plain_text == "Companion")
  // Test: companions.find(x => x.name == "Companion")
